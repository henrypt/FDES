...
const OPTION = {
  ...
  line: {
    ...
    animation: true,
    animationThreshold: 200, // 是否开启动画的阈值，当单个系列显示的图形数量大于这个阈值时会关闭动画
    useUTC: false,
    // 图标题
    title: {
      show: true,
    ...
    xAxis: [
      {
        ...
        // interval: getTimestamp('2014-01-01'), // 强制设置坐标轴分割间隔。因为 splitNumber 是预估的值，实际根据策略计算出来的刻度可能无法达到想要的效果，这时候可以使用 interval 配合 min、max 强制设定刻度划分，一般不建议使用。无法在类目轴中使用。在时间轴（type: 'time'）中需要传时间戳，在对数轴（type: 'log'）中需要传指数值。
        type: 'category',
        nameLocation: 'end',
        nameTextStyle: {
          color: COLORS[0],
  ...
  radar: {
    ...
    tooltip: {
      show: false,
      trigger: 'item', // trigger: 'item', //item: 数据项图形触发. axis: 坐标轴触发
      position: 'inside',
      // formatter(params, ticket) {
      //   return params.name + ' , ' + params.dimensionIndex
      // },
...
const CHART = {
  initOption: (type, title) => {
    ...
    if (type === 'radar') {
      ...
      data.forEach(function (line, index) {
          ...
          option.radar.indicator.push(
            {
              ...
              max:
                index === dataLength - 1
                  ? parseFloat(line.max.value) +
                    parseFloat(line.max.value * 0.2)
                  : line.Max,
              min: 0,
              axisLabel: {
                show: false,
        ...
        // } else
        {
          const seriesDataValue = isConvert ? line.convert : line.value
          const value = seriesDataValue.map((v) => {
            // 空白点处理 当数据值为 NULL 或 ‘’ 空串 时，表示此点为空白点
            const isNullPoint = v === 'NULL' || v === ''
            let isErrorPoint = false
          ...
          const obj = {
            name: line.name,
            value, // isConvert ? line.convert[nindex] : line.value[nindex],
            // 在拐点处显示数值
            label: {
              normal: {
                show: true,
                ...
                formatter: (params, e) => {
                  ...
                  //   params.value +
                  //   '[' +
                  //   DATA[params.dataIndex].names[params.dimensionIndex] +
                  //   ']'
                  // )
                }
              }
           ...
           * 点值
           *  [
           *     {value: [点值1, ..., 点值x], name: '线名称1'}  // 线1
           *     {value: [点值1, ..., 点值x], name: '线名称2'}  // 线2
           *  ]
           */
  ...
  drawPie: (id) => {
    const objMain = document.getElementById(id)
...
export default {
  ...
  mounted () {
    ...
    self.$nextTick(function () {
      ...
      CHART.initOption(CHART_TYPE, self.cinfo.ctitle)
      CHART.drawPie('main')
    })
  ...
  methods: {
    setHeight () {
      this.height = window.innerHeight - window.innerHeight * 0.12
    }
  }
}
</script>
