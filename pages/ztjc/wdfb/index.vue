<template>
  <div>
    <!-- 工具按钮组 -->
    <Buttons v-if="isShowStable" v-model="isShowStable" />

    <!-- <v-card color="basil">
    <v-tabs grow vertical v-model="tab" background-color="green lighten-4">
      <v-tab v-for="item in items" :key="item.tab" class="caption font-weight-bold">
        {{ item.tab }}
      </v-tab>
    </v-tabs>
    ...
    <v-row justify="space-around">
      ...
      <v-col cols="3" class="pdr-0 d-sm-none d-md-none d-lg-flex">
        <v-card style="width:100%;">
          ...
          >
            <v-tab class="subtitle-2 font-weight-bold">
              计算结果
            </v-tab>
            <v-tab class="subtitle-2 font-weight-bold">
              稳定点列表
            ...
            <!-- 计算结果 -->
            <v-tab-item>
              <v-card elevation="2" class="mx-auto mt-1 ml-1 mr-1 mb-2">
                ...
                <v-simple-table dense fixed-header>
                  <template #default>
                    <caption
                      class="text-left caption font-weight-bold sub_title1 pl-2 py-2"
                    >
                      <label v-text="calcInfo.title" />
                      <em class="float-right pr-4 info--text caption">
                        {{ calcInfo.subTitle }}
                      </em>
                    </caption>
                    <thead>
                    ...
                    <tbody>
                      <tr v-for="item in calcInfo.data" :key="item.name">
                        <td>{{ item.name }}</td>
                        <td>{{ item.count }}</td>
                        <td>{{ item.recent }}</td>
                        <td>{{ item.tips }}</td>
                    ...
                    <tfoot>
                      <tr>
                        <td colspan="4" class="border-bottom-solid">
&nbsp;
                        </td>
                      </tr>
                    </tfoot>
                ...
                <v-alert elevation="2" class="overline py-1 text-right">
            ...
            <!-- 稳定点列表 -->
            <v-tab-item>
              <StableList v-model="selectStableList" :single-select="true" />
            </v-tab-item>
          </v-tabs>
        </v-card>
      </v-col>
...
import StableList from '@/components/common/StableList/index.vue'
...
export default {
  ...
  data () {
    return {
      ...
      calcInfo: {
        title: '计算报告:',
        subTitle: ' [基点：2017-01-01 ~ 2019-04-28]',
        data: [
          {
            name: '|8|<=1.5',
            count: 47,
      ...
      xmlData: [
        ...
        },
        {
          ...
          convert: true, // 折算 (注: 默认情况下，雷达图模式下不显示此项)
          conditionLabel: false, // 工况标注 (注: 默认情况下，雷达图模式下不显示此项)
          maskAbnormalData: false, // 屏蔽异常数据  (注: 默认情况下，雷达图模式下不显示此项)
          listTable: true, // 列表
          modal: false, // 模板
  ...
  computed: {
    chartTitle () {
      const index = this.tab || 0
      return this.items[index].tab + '-' + this.title
    },
    chartFunctions () {
      let obj = null
  ...
  methods: {
    countData (self) {
      ...
      self.chartDATA = []
      self.xmlData.forEach((line, index) => {
        ...
        testItem.forEach((item, index) => {
          ...
          lineData.names.push(
            item.name.split('_')[0].replace(/GRH/gi, '') || ''
          ) // 测点简称
